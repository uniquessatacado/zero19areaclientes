...
export function createProductionModule(ctx){
  ...
  async function exportFilm(media,assetMap){
    ...
    const currentExport=()=>!closed&&exportGeneration===productionAccountGeneration&&exportAccount===String(owner()||'');
    ...
    const recordExportOnce=()=>{
      if(!currentExport()||exportRecorded||!ctx.getCostUI?.()||!ctx.onFilmExported)return;
      ...
      exportRecorded=true;ctx.onFilmExported({exportId,items:financeItems,snapshot:filmCostPanel?.getSnapshot?.()||null,companies});
    };
    spotButton.onclick=async()=>{
      if(busy||!currentExport())return;busy=true;spotController=new AbortController();
      const signal=spotController.signal,trim=modal.querySelector('[name="filmExportMode"]:checked').value==='trimmed',errorOutput=modal.querySelector('[data-film-export-error]'),output=modal.querySelector('[data-film-export-results]');
      ...
      try{
        ...
        const result=await exportCanvasWithSpot({name:'filme-completo'+(trim?'-recortado':'')+'-spot.tif',geometry,trim,signal,onProgress:progress,renderCanvas:async({signal,onProgress})=>{
        ...
        output.append(row);spotStatus.hidden=true;ctx.toast(result.saved?'TIFF com Spot salvo. Confira o primeiro arquivo no RIP.':'TIFF com Spot preparado. Toque em “Baixar TIFF com Spot”.','ok');recordExportOnce();
    ...
    modal.querySelector('[data-generate-film-export]').onclick=async()=>{
      if(busy||!currentExport())return;busy=true;const button=modal.querySelector('[data-generate-film-export]'),output=modal.querySelector('[data-film-export-results]'),errorOutput=modal.querySelector('[data-film-export-error]'),trim=modal.querySelector('[name="filmExportMode"]:checked').value==='trimmed';
