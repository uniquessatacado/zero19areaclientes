...
export function createProductionModule(ctx){
  ...
  async function renderFilm(){
    ...
    const drawOrders=()=>{
      ...
      const remaining=linkedRows.filter(row=>pendingQuantity(row,filmItems)>0),count=remaining.reduce((sum,row)=>sum+pendingQuantity(row,filmItems),0),pendingButton=app.querySelector('#addOfficialPending');if(pendingButton)pendingButton.innerHTML='◷ Aguardando produção <b style="margin-left:6px;border-radius:999px;padding:2px 7px;background:#ff6428;color:#111">'+count+'</b>';
