...
export function createProductionModule(ctx){
  ...
  function runNesting(items,options){
    ...
    return new Promise((resolve,reject)=>{
      const worker=new Worker(new URL('./nesting-worker.js?v=2.17.5',import.meta.url),{type:'module'});
  ...
  async function renderFilm(){
    ...
    const drawOrders=()=>{
      ...
      ordersSlot.querySelectorAll('[data-add-order]').forEach(button=>button.onclick=()=>{
        ...
        if(!Number.isSafeInteger(quantity)||quantity<1||quantity>pendingQuantity(row,filmItems))return ctx.toast('Confira a quantidade pendente deste pedido.','err');
  ...
  function filmItemsHTML(){return filmItems.length?filmItems.map(i=>`<article class="film-item editable-film-item"><div class="film-item-thumb">${filmItemPreview(i)}</div><div class="film-item-main"><div class="film-item-head"><div><b>${h(i.label)}</b><small>${i.widthCm.toFixed(2)} × ${i.heightCm.toFixed(2)} cm${i.halftone?' • halftone':''}</small></div><button class="film-remove-item remove-film-item" data-id="${i.localId}" aria-label="Remover ${h(i.label)}">×</button></div><div class="film-item-edit"><label>Quantidade<input class="film-item-qty" data-id="${i.localId}" type="number" inputmode="numeric" min="1" value="${i.quantity}"></label><button class="btn small edit-film-item" data-id="${i.localId}">Tamanho</button><button class="btn small duplicate-film-item" data-id="${i.localId}">Duplicar</button>${hasEditableVectorColor(i)?`<button class="btn small color-film-vector" data-id="${i.localId}">Cor vetor</button>`:''}</div></div></article>`).join(''):'<div class="empty mini">Nenhum item. Adicione uma arte ou personalização.</div>'}
  ...
  async function exportFilm(media,assetMap){
    ...
    let curveLibrary=loadCurveLibrary(owner()),activeCurve={...(curveLibrary.profiles.find(profile=>profile.id===curveLibrary.defaultId)||curveLibrary.profiles[0])};
    const curvePanel=document.createElement('section');curvePanel.className='curve-panel';curvePanel.innerHTML='<h3>Curva CMYK do TIFF</h3><p class="film-export-hint">Altere entrada e saída e confira a estampa em tempo real. A prévia é visual; o TIFF final aplica a curva nos quatro canais CMYK.</p><label class="curve-field">Modelo<select data-curve-profile></select></label><div class="curve-grid"><label class="curve-field">Entrada (%)<span class="curve-value"><input type="range" min="1" max="99" data-curve-input-range><input type="number" min="1" max="99" data-curve-input></span></label><label class="curve-field">Saída (%)<span class="curve-value"><input type="range" min="1" max="99" data-curve-output-range><input type="number" min="1" max="99" data-curve-output></span></label></div><div class="curve-preview"><figure><canvas width="260" height="150" data-curve-original></canvas><figcaption>Arte original</figcaption></figure><figure><canvas width="260" height="150" data-curve-preview></canvas><figcaption>Prévia com a curva</figcaption></figure></div><p class="curve-current" data-curve-current></p><div class="curve-actions"><input class="curve-name" data-curve-name maxlength="60" placeholder="Nome do modelo"><button type="button" class="btn small" data-save-curve>Salvar modelo</button><button type="button" class="btn small" data-default-curve>Fixar para próximas</button><button type="button" class="btn ghost small" data-delete-curve>Excluir modelo</button></div>';
    spotHint.after(curvePanel);
    const curveSelect=curvePanel.querySelector('[data-curve-profile]'),curveInput=curvePanel.querySelector('[data-curve-input]'),curveInputRange=curvePanel.querySelector('[data-curve-input-range]'),curveOutput=curvePanel.querySelector('[data-curve-output]'),curveOutputRange=curvePanel.querySelector('[data-curve-output-range]'),curveName=curvePanel.querySelector('[data-curve-name]'),curveCurrent=curvePanel.querySelector('[data-curve-current]'),originalCanvas=curvePanel.querySelector('[data-curve-original]'),adjustedCanvas=curvePanel.querySelector('[data-curve-preview]');
    let curvePreviewPixels=null;
    const clampCurve=value=>Math.max(1,Math.min(99,Math.round(Number(value)||1)));
    const activeCurveLut=()=>createInkCurveLut(activeCurve.inputPercent,activeCurve.outputPercent);
    const paintCurvePreview=()=>{const lut=activeCurveLut(),graph=adjustedCanvas.getContext('2d');if(curvePreviewPixels){const next=new ImageData(new Uint8ClampedArray(curvePreviewPixels.data),curvePreviewPixels.width,curvePreviewPixels.height);for(let i=0;i<next.data.length;i+=4){for(let channel=0;channel<3;channel++){const ink=255-next.data[i+channel];next.data[i+channel]=255-lut[ink]}}graph.putImageData(next,0,0)}else{graph.clearRect(0,0,adjustedCanvas.width,adjustedCanvas.height);graph.fillStyle='#fff';graph.fillRect(0,0,adjustedCanvas.width,adjustedCanvas.height);graph.strokeStyle='#ff6428';graph.lineWidth=4;graph.beginPath();for(let x=0;x<256;x++){const px=x/255*adjustedCanvas.width,py=adjustedCanvas.height-lut[x]/255*adjustedCanvas.height;if(x)graph.lineTo(px,py);else graph.moveTo(px,py)}graph.stroke()}curveCurrent.textContent=`Em uso nesta impressão: entrada ${activeCurve.inputPercent}% → saída ${activeCurve.outputPercent}%${activeCurve.id===curveLibrary.defaultId?' · padrão das próximas impressões':' · somente esta impressão'}`};
    const syncCurveControls=()=>{curveInput.value=curveInputRange.value=String(activeCurve.inputPercent);curveOutput.value=curveOutputRange.value=String(activeCurve.outputPercent);curveName.value=activeCurve.name||'';curveSelect.value=activeCurve.id;curvePanel.querySelector('[data-delete-curve]').hidden=activeCurve.id===STANDARD_CURVE_ID;paintCurvePreview()};
    const refreshCurveSelect=()=>{curveSelect.innerHTML=curveLibrary.profiles.map(profile=>`<option value="${h(profile.id)}">${h(profile.name)}${profile.id===curveLibrary.defaultId?' · padrão':''}</option>`).join('');syncCurveControls()};
    const changeCurve=(field,value)=>{activeCurve={...activeCurve,[field]:clampCurve(value)};syncCurveControls()};
    curveInput.oninput=curveInputRange.oninput=event=>changeCurve('inputPercent',event.target.value);curveOutput.oninput=curveOutputRange.oninput=event=>changeCurve('outputPercent',event.target.value);
    curveSelect.onchange=()=>{activeCurve={...(curveLibrary.profiles.find(profile=>profile.id===curveSelect.value)||curveLibrary.profiles[0])};syncCurveControls()};
    curvePanel.querySelector('[data-save-curve]').onclick=()=>{const name=String(curveName.value||'').trim();if(!name)return ctx.toast('Dê um nome para salvar o modelo de curva.','err');const id=activeCurve.id===STANDARD_CURVE_ID?'curve-'+Date.now():activeCurve.id;curveLibrary=upsertCurveProfile(curveLibrary,{...activeCurve,id,name,builtIn:false});activeCurve={...curveLibrary.profiles.find(profile=>profile.id===id)};refreshCurveSelect();ctx.toast('Modelo de curva salvo.','ok')};
    curvePanel.querySelector('[data-default-curve]').onclick=()=>{let selected=curveLibrary.profiles.find(profile=>profile.id===activeCurve.id);if(!selected||selected.inputPercent!==activeCurve.inputPercent||selected.outputPercent!==activeCurve.outputPercent){const id='curve-'+Date.now(),name=String(curveName.value||`Curva ${activeCurve.inputPercent} → ${activeCurve.outputPercent}`).trim();curveLibrary=upsertCurveProfile(curveLibrary,{...activeCurve,id,name,builtIn:false});activeCurve={...curveLibrary.profiles.find(profile=>profile.id===id)}}curveLibrary=setDefaultCurve(curveLibrary,activeCurve.id);refreshCurveSelect();ctx.toast('Curva fixada para as próximas impressões deste computador.','ok')};
    curvePanel.querySelector('[data-delete-curve]').onclick=()=>{curveLibrary=removeCurveProfile(curveLibrary,activeCurve.id);activeCurve={...(curveLibrary.profiles.find(profile=>profile.id===curveLibrary.defaultId)||curveLibrary.profiles[0])};refreshCurveSelect();ctx.toast('Modelo de curva excluído.','ok')};
    refreshCurveSelect();
    const previewItem=items.find(item=>item.previewDataUrl||item.path),previewSource=previewItem?.previewDataUrl||(previewItem?.path?ctx.publicUrl(previewItem.path):'');
    if(previewSource){const image=new Image();image.crossOrigin='anonymous';image.onload=()=>{try{const context=originalCanvas.getContext('2d'),ratio=Math.min(originalCanvas.width/image.naturalWidth,originalCanvas.height/image.naturalHeight),width=image.naturalWidth*ratio,height=image.naturalHeight*ratio,x=(originalCanvas.width-width)/2,y=(originalCanvas.height-height)/2;context.fillStyle='#fff';context.fillRect(0,0,originalCanvas.width,originalCanvas.height);context.drawImage(image,x,y,width,height);curvePreviewPixels=context.getImageData(0,0,originalCanvas.width,originalCanvas.height);paintCurvePreview()}catch{curvePreviewPixels=null;paintCurvePreview()}};image.onerror=()=>paintCurvePreview();image.src=previewSource}else paintCurvePreview();
    ...
    spotButton.onclick=async()=>{
      ...
      for(const url of urls)URL.revokeObjectURL(url);urls.clear();output.innerHTML='';errorOutput.textContent='';modal.querySelectorAll('button,input,select').forEach(control=>control.disabled=true);spotStatus.hidden=false;
      ...
      try{
        ...
        const curveSnapshot={...activeCurve},curveLut=activeCurveLut();
        const result=await exportCanvasWithSpot({name:filmExportBase(media)+'-spot.tif',geometry,trim,signal,curveLut,onProgress:progress,renderCanvas:async({signal,onProgress})=>{
      ...
      finally{busy=false;spotController=null;modal.querySelectorAll('button,input,select').forEach(control=>control.disabled=false)}
    };
