...
function filterAssets(){
  const q=($('#assetSearch')?.value||'').toLowerCase(), t=$('#typeFilter')?.value||'all'; let list=currentAssets.filter(a=>(activeFolder==='all'||(activeFolder==='root'&&!a.folder_id)||a.folder_id===activeFolder)&&(t==='all'||a.asset_type===t)&&a.name.toLowerCase().includes(q)); $('#assetGrid').innerHTML=renderAssets(list);bindAssetCards();bindEmptyActions();
}
...
ensureDefaults = async function(){
  if(!session?.user?.id)return; if(!currentProfile)await loadTeamContext(); const uid=accountOwnerId();
  const [{count:statusCount},{count:productCount},{count:folderTemplateCount}]=await Promise.all([
    supabase.from('z19p_statuses').select('id',{count:'exact',head:true}).eq('owner_id',uid),
    supabase.from('z19p_products').select('id',{count:'exact',head:true}).eq('owner_id',uid),
    supabase.from('z19p_folder_templates').select('id',{count:'exact',head:true}).eq('owner_id',uid)
...
ensureLibraries = async function(){
  if(!session?.user?.id)return; const uid=accountOwnerId();
  const {data,error}=await supabase.from('z19p_workspaces').select('*').eq('owner_id',uid).in('workspace_type',['library_artes','library_mockups','library_videos']);if(error){console.error(error);return;}
  ...
  if(missing.length)await supabase.from('z19p_workspaces').insert(missing);
  const {data:fresh}=await supabase.from('z19p_workspaces').select('*').eq('owner_id',uid).in('workspace_type',['library_artes','library_mockups','library_videos']);libraryWorkspaces=Object.fromEntries((fresh||[]).map(w=>[w.workspace_type,w]));
};
...
renderWorkspaceCards = function(items){
  if(!items.length)return `<div class="empty" style="grid-column:1/-1"><div class="big">◫</div><b>Nenhuma empresa encontrada</b><div>Crie uma empresa ou altere os filtros.</div></div>`;
  return sortWorkspacesByAttention(items).map(w=>{
    const st=statusById(w.status_id),attention=workspaceStatusAttention(w),age=workspaceStatusDuration(w),rem=reminderInfo(w);
    const logo=w._logo_url?`<img src="${w._logo_url}" alt="Logo ${escapeHTML(w.company_name)}">`:'019';
    ...
    const alert=attention?`<div class="status-aging-alert"><span class="attention-pulse"></span><div><b>${attention.reminder?escapeHTML(attention.label):`${escapeHTML(attention.label)} sem alteração de status`}</b><small>${w.phone?'Chame no WhatsApp e atualize o andamento.':'Revise o atendimento e atualize o status.'}</small></div></div>`:'';
    const pull=!isFinalizedStatus(st)&&w.responsible_user_id!==session.user.id?`<button class="btn small claim-workspace" data-id="${w.id}">Puxar pra mim</button>`:'';
    return `<article class="workspace-card clean ${attention?'needs-status-attention':''} ${isDesistedStatus(st)?'is-desisted':''} ${rem?.due?'reminder-due':''}"><div class="workspace-head"><div class="workspace-icon ${w._logo_url?'has-logo':''}">${logo}</div><div class="workspace-copy"><div class="workspace-name-line"><h3>${escapeHTML(w.company_name)}</h3>${w.state?`<span class="workspace-state">${escapeHTML(w.state)}</span>`:''}</div><div class="meta">${escapeHTML(w.client_name||'Cliente não informado')}</div>${w.phone?`<div class="meta">${escapeHTML(w.phone)}</div>`:''}</div></div>${sellerBadge(w)}${workspaceSpecialCallout(w)}${alert}<div class="card-status"><label>Status</label><select class="status-select card-status-select" data-id="${w.id}"><option value="">Sem status</option>${statusOptions(w.status_id)}</select><span class="status-dot" style="--status:${escapeHTML(st?.color||'#555')}"></span></div>${ageLine}<div class="card-actions">${pull}${w.phone?`<button class="btn whatsapp small wa-workspace ${attention?'attention-wa':''}" data-id="${w.id}">WA <span>WhatsApp</span></button>`:''}<button class="btn primary small open-workspace" data-id="${w.id}">Abrir</button><button class="btn ghost small workspace-more" data-id="${w.id}">•••</button></div></article>`;
  }).join('');
};
...
function dashboardClientListsHTML(items=workspaces){
  const urgent=sortWorkspacesByAttention(items.filter(w=>workspaceStatusAttention(w))),regular=items.filter(w=>!workspaceStatusAttention(w));
  return `<section id="attentionClients" class="attention-clients ${urgent.length?'':'hidden'}"><div class="client-section-head"><div><div class="eyebrow">Ação necessária</div><h2>Atendimentos que precisam de atenção</h2></div><span id="attentionCount">${urgent.length}</span></div><div id="attentionGrid" class="grid workspace-grid-clean priority-grid">${urgent.length?renderWorkspaceCards(urgent):''}</div></section><section class="clients-section"><div class="client-section-head"><div><div class="eyebrow">Clientes</div><h2>Empresas e clientes</h2></div></div><div id="workspaceGrid" class="grid workspace-grid-clean">${regular.length?renderWorkspaceCards(regular):(urgent.length?'<div class="empty mini" style="grid-column:1/-1">Todos os atendimentos visíveis estão na lista de atenção acima.</div>':renderWorkspaceCards([]))}</div></section>`;
}
...
renderQuotesAdmin = function(){
  if(!currentQuotes.length)return `<div class="quote-empty"><div><b>Nenhum orçamento criado</b><span>Monte o orçamento e ele aparecerá automaticamente na área do cliente.</span></div><button class="btn primary small" id="newQuote">${icon('plus')} Criar orçamento</button></div>`;
  return `<div class="quote-list">${currentQuotes.map(q=>`<article class="quote-summary"><div><div class="quote-title-line"><b>${escapeHTML(q.title||'Orçamento')}</b><span>${fmtMoney(quoteTotal(q))}</span></div><small>${q.items?.length||0} item(ns)${q.delivery_term?` • Prazo: ${escapeHTML(q.delivery_term)}`:''} • por ${escapeHTML(profileName(q.created_by))}</small></div><div class="quote-actions"><button class="btn small edit-quote" data-id="${q.id}">${icon('edit')} Editar</button><button class="btn ghost small delete-quote" data-id="${q.id}">${icon('trash')}</button></div></article>`).join('')}</div>`;
};
