...
const pxAt300 = cm => Math.max(1,Math.round((Number(cm)||0)/2.54*300));
...
async function processQueue(m,{mockupMode=false}={}){
  ...
  for(const q of queue){
    try{
      ...
      const isMockup=q.type==='mockup',doTrim=isMockup?false:trim,qualityTarget=0;
      const result=await processImage(q.file,{trim:doTrim,targetWidthCm:isMockup?0:Number(String(q.widthCm||'').replace(',','.'))||0,targetHeightCm:isMockup?0:Number(String(q.heightCm||'').replace(',','.'))||0,onStage:text=>stage(`${done+1}/${queue.length} · ${text}`)});
...
async function processImage(file,{trim,targetMax=0,targetWidthCm=0,targetHeightCm=0,onStage=()=>{}}={}){
  ...
  let physicalScale=0,targetPixels=null;
  if(targetWidthCm>0){const px=pxAt300(targetWidthCm);physicalScale=px/workingWidth;targetPixels={axis:'width',pixels:px,cm:targetWidthCm}}
  else if(targetHeightCm>0){const px=pxAt300(targetHeightCm);physicalScale=px/workingHeight;targetPixels={axis:'height',pixels:px,cm:targetHeightCm}}
  const requestedScale=physicalScale>0?physicalScale:(targetMax>0?targetMax/Math.max(workingWidth,workingHeight):0);
  if(requestedScale>1.001){
    const scale=requestedScale,w=Math.max(1,Math.round(workingWidth*scale)),h=Math.max(1,Math.round(workingHeight*scale)),touchDevice=(navigator.maxTouchPoints||0)>1||/iPhone|iPad|iPod|Android/i.test(navigator.userAgent||'');
    outputWidth=w;outputHeight=h;onStage(`Preparando ${w} × ${h} px para 300 DPI…`);
  ...
  return{blob,width:outputWidth,height:outputHeight,sourceWidth,sourceHeight,upscaled,engine,targetPixels};
}
...
renderWorkspaceCards = function(items){
  ...
  return customerView(items,{ownerId:accountOwnerId(),userId:session?.user?.id,scope:'all',projects:currentProjects,statuses}).map(w=>{
    ...
    return `<article class="workspace-card clean customer-card ${w.responsible_user_id===session?.user?.id?'mine':''} ${info.attention?'needs-status-attention':''} ${info.closed?'is-closed':''}" data-workspace="${escapeHTML(w.id)}"><div class="workspace-head"><div class="workspace-icon ${w._logo_url?'has-logo':''}">${logo}</div><div class="workspace-copy"><div class="workspace-name-line"><h3>${escapeHTML(w.company_name)}</h3>${w.state?`<span class="workspace-state">${escapeHTML(w.state)}</span>`:''}</div><div class="meta">${escapeHTML(w.client_name||'Cliente não informado')}</div></div></div><div class="customer-order-line"><span class="customer-service">${escapeHTML(info.serviceLabel)}</span>${statusPill(st)}</div>${delivery}${alert}<details class="customer-secondary"><summary>Status e responsável</summary>${sellerBadge(w)}${w.phone?`<div class="meta">${escapeHTML(w.phone)}</div>`:''}${info.closed?'':workspaceSpecialCallout(w)}<div class="card-status"><label for="customer-status-${escapeHTML(w.id)}">Alterar status</label><select id="customer-status-${escapeHTML(w.id)}" class="status-select card-status-select" data-id="${w.id}"><option value="">Sem status</option>${statusOptions(w.status_id)}</select></div>${ageLine}${pull}</details><div class="card-actions"><button class="btn primary small open-workspace" data-id="${w.id}">${info.closed?'Ver histórico':'Abrir pedido'}</button>${w.phone?`<button class="btn small wa-workspace" data-id="${w.id}">WhatsApp</button>`:''}<button class="btn ghost small workspace-more" data-id="${w.id}" aria-label="Mais opções de ${escapeHTML(w.company_name)}">•••</button></div></article>`;
  }).join('');
};
...
function dashboardClientListsHTML(items=workspaces,{searching=false}={}){
  if(searching){
    return `<section class="clients-section search-results-section"><div class="client-section-head"><h2>Resultados da busca</h2><span>${items.length}</span></div><div class="grid workspace-grid-clean">${renderWorkspaceCards(items)}</div></section>`;
  }
...
renderDashboard = async function(){
  ...
  const identity=`${accountOwnerId()}:${session.user.id}`;if(identity!==dashboardCustomerIdentity){dashboardCustomerIdentity=identity;dashboardCustomerScope=isAdmin()?'all':'mine';}
  const dashboardSearchQuery=()=>String($('#workspaceSearch')?.value||'').trim();
  const filtered=()=>{const query=dashboardSearchQuery();return customerView(workspaces,{ownerId:accountOwnerId(),userId:session.user.id,scope:query?'all':dashboardCustomerScope,projects:currentProjects,statuses,query,statusId:query?'all':($('#statusFilter')?.value||'all')});};
...
officialOrders=createOfficialOrderWorkflow({
  ...
  onSaved:async()=>{zero19Sync?.invalidate?.();if(currentWorkspace?.id)await renderWorkspace(currentWorkspace.id);}
});
