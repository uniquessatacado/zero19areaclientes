...
function openUploadModal({presetType='arte'}={}){
  ...
  $('#uploadCreateFolder',m)?.addEventListener('click',()=>safe(async()=>{const name=prompt('Nome da nova pasta:');if(!name?.trim())return;const projectId=currentWorkspace?.workspace_type==='client'?(currentProjects[0]?.id||null):null;const {data,error}=await supabase.from('z19p_folders').insert({owner_id:accountOwnerId(),workspace_id:currentWorkspace.id,project_id:projectId,parent_id:null,name:name.trim(),sort_order:(currentFolders.length+1)*10,created_by:session.user.id,updated_by:session.user.id}).select().single();if(error)throw error;currentFolders=[...currentFolders,data];for(const item of uploadQueue)if(item.type==='arte'&&!item.folderId)item.folderId=data.id;renderUploadList(m,{lockedType:isMockupMode});toast('Pasta criada e selecionada.','ok')}));
