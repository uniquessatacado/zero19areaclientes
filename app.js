...
shell = function(content,{back=false}={}){
  ...
  return `<div class="app-shell"><header class="topbar"><button class="nav-menu-trigger" type="button" data-drawer-open aria-label="Abrir menu" aria-controls="appDrawer">&#9776;</button>${back?`<button class="btn ghost small top-back" data-nav="/" aria-label="Voltar ao início">${icon('back')}</button>`:''}<div class="brand" data-nav="/" role="button" tabindex="0" aria-label="Ir para o início">${brandLogoHTML()}</div><div class="top-actions"><span class="app-version-badge" title="Versão do sistema">v${APP_VERSION}</span>${who}${session?`<button class="btn ghost small" id="logoutBtn">${icon('logout')} <span class="label">Sair</span></button>`:''}</div></header>
    <div class="app-drawer-backdrop" data-drawer-close></div><aside class="app-drawer" id="appDrawer" aria-hidden="true"><div class="drawer-head">${brandLogoHTML('drawer-logo')}<button type="button" data-drawer-close aria-label="Fechar menu">×</button></div><nav><div class="drawer-section"><small>Principal</small><button class="${active('/')}" data-nav="/">Clientes e empresas</button><button class="${active('/empresas-parceiras')}" data-nav="/empresas-parceiras">Empresas parceiras</button><button class="${active('/links')}" data-nav="/links">Links para compartilhar</button></div><div class="drawer-section"><small>Produção</small>${canViewProductionCosts()?`<button class="${active('/financeiro-impressao')}" data-nav="/financeiro-impressao">Financeiro de impressão · privado</button>`:''}${garmentStudioEnabled?`<button class="${active('/studio')}" data-nav="/studio">Estúdio de mockups</button>`:''}<button class="${active('/filme')}" data-nav="/filme">Montar filme DTF</button><button class="${active('/times')}" data-nav="/times">Times / Personalizações</button><button data-nav="/fila/art_work">Fila: Desenvolver arte</button><button data-nav="/fila/ready_production">Fila: Iniciar produção</button><button data-nav="/fila/production">Fila: Estampar</button></div><div class="drawer-section"><small>Bibliotecas</small><button data-nav="/biblioteca/artes">Artes</button><button data-nav="/biblioteca/videos">Vídeos</button><button data-nav="/biblioteca/mockups">Mockups</button></div>${adminMenu}</nav><div class="drawer-user">${who}<span>v${APP_VERSION}</span></div></aside>
    ${content}
...
renderWorkspaceCards = function(items){
  ...
  return items.map(w=>{const client=clientDisplayName(w),company=optionalCompanyName(w),resp=profileById(w.responsible_user_id);return `<article class="workspace-card clean client-card-simple"><div class="workspace-head"><div class="workspace-icon"><img class="default-card-brand" src="${BRAND_LOGO}" alt="Zero 19"></div><div class="workspace-copy"><div class="workspace-name-line"><h3>${escapeHTML(client)}</h3></div>${company?`<div class="meta">${escapeHTML(company)}</div>`:''}${w.phone?`<div class="meta">${escapeHTML(w.phone)}</div>`:''}<div class="meta client-responsible">Responsável: ${escapeHTML(resp?.full_name||'Não definido')}</div></div></div><div class="card-actions">${w.phone?`<button class="btn whatsapp small wa-workspace" data-id="${w.id}">WA <span>WhatsApp</span></button>`:''}<button class="btn primary small open-workspace" data-id="${w.id}">Abrir</button><button class="btn ghost small workspace-more" data-id="${w.id}" aria-label="Opções">•••</button></div></article>`}).join('');
};
bindWorkspaceCards = function(){
  $$('.open-workspace').forEach(b=>b.onclick=()=>nav(`/ambiente/${b.dataset.id}`));$$('.workspace-more').forEach(b=>b.onclick=()=>openWorkspaceActions(workspaces.find(w=>w.id===b.dataset.id)));$$('.wa-workspace').forEach(b=>b.onclick=()=>openWhatsApp(workspaces.find(w=>w.id===b.dataset.id)?.phone));
};
...
renderWorkspace=async function(id){const stillCurrent=accountReadGuard(true);const ticket=!renderingRoute?routeViewport.begin(route()):null;await renderWorkspaceV216(id);if(!stillCurrent())return;if(currentWorkspace?.id===id&&currentWorkspace.workspace_type==='client'){await loadProjects();if(!stillCurrent())return;const w=currentWorkspace,banner=app.querySelector('.workspace-banner'),client=clientDisplayName(w),company=optionalCompanyName(w),resp=profileById(w.responsible_user_id);banner?.querySelector('h1')&&(banner.querySelector('h1').textContent=client);const meta=banner?.querySelector('.meta');if(meta)meta.textContent=[company,w.phone,resp?.full_name?'Responsável: '+resp.full_name:null].filter(Boolean).join(' • ');app.querySelector('#mockupBtn')?.remove();app.querySelector('#workspaceMore')?.remove();app.querySelector('.workspace-aging-notice')?.remove();app.querySelector('.workspace-control-row')?.remove();app.querySelector('.quotes-admin')?.remove();const sidebar=app.querySelector('.workspace-layout .sidebar');sidebar?.remove();const layout=app.querySelector('.workspace-layout');layout?.classList.add('client-simple-layout');app.querySelector('#typeFilter')?.remove();const projects=currentProjects.filter(p=>p.workspace_id===id).sort((a,b)=>Number(b.sequence_no||0)-Number(a.sequence_no||0));const projectHtml=projects.length?projects.map(p=>{const pr=profileById(p.responsible_user_id),date=p.started_at?new Date(p.started_at).toLocaleDateString('pt-BR'):'';return `<article class="client-project-row"><div><b>${escapeHTML(p.title||('Projeto '+(p.sequence_no||'')))}</b><small>${date}${pr?.full_name?' • '+escapeHTML(pr.full_name):''}</small></div><span>Pedido / projeto</span></article>`}).join(''):'<div class="empty mini">Nenhum pedido sincronizado ainda.</div>';banner?.insertAdjacentHTML('afterend',`<section class="client-projects-simple"><div class="section-title-row"><div><div class="eyebrow">Projetos</div><h2>Pedidos do cliente</h2><p>Esta lista será alimentada automaticamente pela integração com o outro sistema.</p></div></div><div class="client-project-list">${projectHtml}</div></section>`);}if(ticket)routeViewport.complete(ticket);};
