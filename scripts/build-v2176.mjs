...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'z19p-v2177-'));
...
function replaceOnce(before,after){const count=production.split(before).length-1;if(count!==1)throw new Error('Pre-build v2.17.7: trecho alterado ou ausente ('+count+'): '+before.slice(0,100));production=production.replace(before,after);}
...
production="import {refineLetteringLayout} from './lettering-layout-v2176.js?v=2.17.7';\nimport {createSavedFilmDrafts,openSavedFilmDialog,filmNameProof,NAMED_FILM_DRAFT_KIND} from './film-saved-drafts.js?v=2.17.7';\n"+production;
...
production=production.replaceAll('2.17.5','2.17.7');
fs.writeFileSync(path.join(temp,'production-v217.js'),production);
for(const name of ['app.js','index.html'])fs.writeFileSync(path.join(temp,name),fs.readFileSync(path.join(temp,name),'utf8').replaceAll('2.17.5','2.17.7'));
const readme=path.join(temp,'README.md');fs.writeFileSync(readme,fs.readFileSync(readme,'utf8')+'\nBuild estático v2.17.7: altura-base, espaçamento óptico e rascunhos nomeados. TIFF/Spot permanece pendente.\n');
...
if(!fs.readFileSync(path.join(temp,'index.html'),'utf8').includes('z19-version" content="2.17.7"'))throw new Error('Versão de saída inválida.');
...
console.log('V2177_VALIDATION_DIR='+temp);
console.log('V2177_OUTPUT_HASHES='+JSON.stringify(hashes));
