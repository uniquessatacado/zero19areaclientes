...
const temp=fs.mkdtempSync(path.join(os.tmpdir(),'z19p-v2176-'));
...
function replaceOnce(before,after){const count=production.split(before).length-1;if(count!==1)throw new Error('Pre-build v2.17.6: trecho alterado ou ausente ('+count+'): '+before.slice(0,100));production=production.replace(before,after);}
...
production="import {refineLetteringLayout} from './lettering-layout-v2176.js?v=2.17.6';\nimport {createSavedFilmDrafts,openSavedFilmDialog,filmNameProof,NAMED_FILM_DRAFT_KIND} from './film-saved-drafts.js?v=2.17.6';\n"+production;
...
replaceOnce("      name:cleanName,number:cleanNumber,fontSource:set._source||null,fontSources:set._sources||[],","      name:cleanName.normalize('NFC'),number:cleanNumber,letteringMetricsVersion:2,spacingMode:'optical',fontSource:set._source||null,fontSources:set._sources||[],");
...
replaceOnce("label:set._path||customizationName(set),quantity,halftone:Boolean(set.halftone),allowInternalNesting:false,rotationPolicy:'none'","label:set._path||customizationName(set),quantity,letteringMetricsVersion:2,spacingMode:'optical',halftone:Boolean(set.halftone),allowInternalNesting:false,rotationPolicy:'none'");
...
replaceOnce("      drawFilm();app.querySelector('#exportFilm').disabled=false;app.querySelector('#saveFilm').disabled=false;",`      for(const item of filmItems){const control=app.querySelector('.film-item-qty[data-id="'+item.localId+'"]'),line=control?.closest('article')?.querySelector('small');if(line)line.textContent=item.widthCm.toFixed(2)+' × '+item.heightCm.toFixed(2)+' cm'+(item.name?' · corpo da letra '+Number(item.nameHeightCm).toLocaleString('pt-BR')+' cm':'');}
...
production=production.replaceAll('2.17.5','2.17.6');
fs.writeFileSync(path.join(temp,'production-v217.js'),production);
for(const name of ['app.js','index.html'])fs.writeFileSync(path.join(temp,name),fs.readFileSync(path.join(temp,name),'utf8').replaceAll('2.17.5','2.17.6'));
const readme=path.join(temp,'README.md');fs.writeFileSync(readme,fs.readFileSync(readme,'utf8')+'\nBuild estático v2.17.6: altura-base, espaçamento óptico e rascunhos nomeados. TIFF/Spot permanece pendente.\n');
for(const name of ['app.js','production-v217.js','lettering-layout-v2176.js','film-saved-drafts.js'])execFileSync(process.execPath,['--check',name],{cwd:temp,stdio:'inherit'});
...
if(!fs.readFileSync(path.join(temp,'index.html'),'utf8').includes('z19-version" content="2.17.6"'))throw new Error('Versão de saída inválida.');
...
console.log('V2176_VALIDATION_DIR='+temp);
console.log('V2176_OUTPUT_HASHES='+JSON.stringify(hashes));
