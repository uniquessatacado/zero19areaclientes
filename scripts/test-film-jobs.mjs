...
function harness({items=[item()],projects={a1:'project-1'},failItems=false,failCleanup=false,gate=null}={}){
  ...
  const factory=new Function('supabase','app','ctx','initialItems','initialLayout','console','rotatedBoundsMm','runtime','location','document','window',`let filmItems=initialItems,lastFilm=initialLayout,filmSettings={},productionAccountGeneration=0,filmDraftChange=0; const owner=()=>runtime.account,uid=()=> 'user',h=String,suggestedFilmName=()=> 'Filme 28 cm — Teste — 25-09 00h00',renderFilm=async()=>{runtime.renders++;return runtime.onRender?await runtime.onRender():true},prepareLetteringLayout=async()=>{runtime.preparations++;if(runtime.prepareGate)await runtime.prepareGate;return {widthCm:20,heightCm:30}}; ${code};return {saveFilmJob,openFilmJob,checkedFilmSnapshot,reviewLegacyFilmLettering,bumpAccountGeneration:()=>productionAccountGeneration++,bumpDraftChange:()=>filmDraftChange++,replaceDraft:(items,layout,settings={})=>{filmItems=items;lastFilm=layout;filmSettings=settings;filmDraftChange++},current:()=>({items:filmItems,layout:lastFilm,settings:filmSettings})};`);
