...
- Liberação exige orçamento válido e preparação aplicável no banco, não só na interface. Entregas históricas permanecem encerradas sem novas pendências retroativas. Não inventar valores, pagamentos ou artes.
- Filme deverá sugerir saldos liberados por pedido/arte e registrar quantidades apenas depois de gerar arquivo com sucesso. Reexportações PNG/TIFF idempotentes; geração não comprova impressão física nem entrega. Preservar edição livre, medidas, Spot e exclusão sem reorganização da 2.17.10.
- Base limpa: `a5f5b09`, produção confirmada 2.17.10. Mudanças de dados aditivas com isolamento de conta e validação transacional. Esta seção não significa publicação; registrar evidências e pendências reais ao concluir.
- Implementados `quote-service-core/ui`: novos orçamentos com produto, arte/DTF, aplicação e remoção independentes; preço do catálogo com ajuste só no orçamento; artes de toda a empresa, sem exigir finalização para orçar. Novos pagos preservam preços/quantidades/referências, permitindo preparar finais e mockup. Orçamentos legados pagos ficam em consulta; não são convertidos automaticamente.
- Mockup oficial: oito cores padrão + cores personalizadas reutilizáveis, modelos normal/over, controles secundários recolhidos, imagens originais preservadas. Bridge orçamento→mockup carrega todas as finais com caminhos e medidas atuais; salva um novo asset `mockup` anotado, com metadados de camadas/posições em cm. Liberação exige a presença de cada final e medidas do mockup compatíveis com o perfil atual (tolerância 0,01 cm).
- Mobile: campos >=16 px, alvos de toque >=44 px, VisualViewport para teclado, sem bloquear pinch acessível ou zoom dos editores; filtros Meus/Todos limitados à mesma conta, ativos por prazo e concluídos recolhidos. **Removido o bloco “O que você veio fazer?” a pedido expresso do usuário.** As filas continuam acessíveis pelo menu; não remover o aviso de saldos na montagem do filme.
- Banco aplicado em `kedggjyerexnzmipaick`: migration remota `20260921012637_v21711_services_production`, com arquivo local alinhado à versão atribuída pelo MCP. Colunas aditivas, duas tabelas de exportação/alocação com RLS/leitura de equipe e escrita só pela RPC privada autenticada. Sem backfill de valores, pagamentos, artes ou entregas. Verificados RLS, ausência de leitura anon/escrita direta autenticada, RPC pública invoker e zero alocações após migration. Advisors: nenhum alerta de segurança novo no escopo z19p; apenas INFO dos dez índices recém-criados ainda sem uso. Alertas históricos de outras aplicações no mesmo banco não alterados.
- Pagamento/salvamento: bloqueio de alterações financeiras em itens pagos também por escrita direta; locks projeto→orçamento e confirmação de pagamento idempotente preservam cálculo existente de comissões. Liberação validada no banco por serviço, pagamento/data, finais, medidas e mockup; finalizados históricos não recebem exigências retroativas.
- Filme: saldo por pedido/arte, inclusão rápida com vínculo, registro transacional só após geração bem-sucedida; falha/cancelamento não consome. Identidade independente de PNG/TIFF e high-water por item impedem duplicação em reexportações. Pedidos parciais continuam na lista; não se presume impressão física, estoque ou entrega. Resposta inválida exige retentativa com a mesma chave. Montagem que já contém itens registrados não oferece novamente custo integral automático; novos custos incrementais exigem conferência do responsável.
- **Limite legado explícito:** pedidos antigos liberados sem vínculo operacional por arte/quantidade aparecem em “Pedidos antigos para conferir”, com acesso ao pedido, sem inventar saldos nem alterar orçamento pago. A contagem automática de produção funciona nos novos itens vinculados. Vincular operacionalmente esses antigos requer fluxo específico futuro; não alegar migração automática concluída.
- Gate local: 53 suítes Node + validação estática; PostgreSQL isolado/PGlite 0.5.8 com dados sintéticos e 27 casos ampliados, mais cenários base de RLS/conta, parcial/replay/fechamento, passou. Teste não demonstra concorrência real de duas conexões nem sessão autenticada de produção. Dependência de QA privada em `tmp`, não publicada. Build final `z19p-v21711-oCDesE`; SHA256 production compilada `59cb5b04f8cb01019f0dc8141d587d560c1b354678b2ea64b921a0dea3db0303`.
- Edge final desktop/mobile: pedidos `z19-order-film-results-cfdwsu/report.json`; regressão filme `z19-film-results-wLgAOC/report.json`. PNG integral→recorte zero diferenças; Spot zero divergências em 8.761.150 pixels integral e 3.957.226 recorte. Worker/PNG/TIFF reais, RPC de pedidos simulada. Smoke login e páginas públicas passou, sem erros JS, imagens quebradas ou overflow. iPhone/teclado físico real e impressão/RIP ainda não testados; não prometer perfeição universal.
- Localhost final: Python oculto PID `27188`, porta 8080, pasta `dist`; HTTP200/meta2.17.11 e ausência do launcher confirmados. Manter rodando. QA temporária separada em 8087. GitHub/Vercel desta versão ainda devem ser confirmados antes de registrar como publicada.
- QA final adicional: orçamento desktop/mobile `z19-quote-results-KU5uMR/report.json`; mockup `z19-garment-results-7dC7uo` com 24 vistas e 12 comparações de cor por viewport, custom colors e fluxo de duas finais; clientes `z19-customer-mobile-cyixLS/report.json`, larguras 1280/390/320 e viewport reduzida de teclado. Captura inicial limpa em `z19-customer-mobile-7uTsup`. Primeiro bootstrap simultâneo da fixture garment expirou, repetição isolada passou sem alteração de código; registrar como limite de execução, não ocultar. Browser integrado indisponível e CLI agent-browser ausente; usados scripts Edge/CDP isolados existentes, sem ler perfil/sessão do usuário.
- **Publicação confirmada:** implementação `a7c98d0010f30ee27503c8daec4700c194415134` enviada para `main`. Preview `dpl_4oCpcquS9x63npr8nXkqQSqGXuek` / `019-personalizacoes-78usy4tsb-uniquess.vercel.app` passou as 53 suítes na Vercel (build 11 s), com hashes iguais aos locais; smoke executável passou via acesso temporário oficial sem desativar proteção. Promoção criou produção `dpl_6HsRyr9AXd4p38XT8BAz32XiLs7z`, **READY**, URL `019-personalizacoes-659jsof1k-uniquess.vercel.app`, framework estático/Other, aliases oficiais confirmados.
- Pós-deploy em `https://019-personalizacoes.vercel.app/`: HTML e 17 módulos/estilos conferidos byte a byte contra `dist`, todos HTTP200. Login desktop1280/celular390, páginas públicas e redirects passaram com zero exceções JS, imagens quebradas ou overflow. Logs error/fatal do deployment desde 1h sem registros; isso não é monitoramento integral de sessões autenticadas (app estático). Não houve prova física de impressão nem Safari de iPhone real. Pedidos legados continuam com limitação explicitada acima.
- Novo diagnóstico INFO do advisor limitado a [índices recém-criados sem uso](https://supabase.com/docs/guides/database/database-linter?lint=0005_unused_index), esperado antes do tráfego das novas funções. Nenhum alerta de segurança novo no escopo desta versão. Não apagar índices que protegem FKs/consultas apenas por ainda não terem estatísticas de uso.

## v2.17.10 — adição de artes e carregamento do filme (publicada em 20/09/2026)

- Relato após 2.17.9: adicionar artes continua lento e a tela mostra `TypeError: Failed to fetch`.
- Diagnóstico local: recursos estáticos essenciais HTTP 200/304, sem divergência do build. A adição ainda executava `renderFilm`, repetindo consultas de toda a biblioteca, e preparava máscaras sequencialmente. A consulta autenticada específica que falhou no navegador do usuário ainda não foi identificada; não atribuir a causa a uma indisponibilidade externa comprovada.
- Escopo: atualizar itens no editor sem recarregar a página/biblioteca; preparar máscaras com concorrência limitada e deduplicação, preservando todo alpha positivo; carregar resumos leves de jobs e buscar o conteúdo só ao abrir. Erros devem informar a etapa, mantendo a montagem existente quando possível.
- Preservados remoção sem reorganizar, medidas, rotação, travas, rascunhos, conta/RLS, PNG original e TIFF CMYK + Spot. Nenhuma alteração de banco ou envio de artes a terceiros. Publicação online verificada conforme evidências abaixo.
- Implementados atualização síncrona dos itens sem SELECT/reconstrução do editor, preparação em segundo plano, pool de duas máscaras com deduplicação por fonte e LRU 60, preservando alpha positivo de um único pixel. Cache de receitas de nomes/números limitado a 32, com invalidação por conteúdo/conta e sem alterar dimensões ou métricas.
- Jobs usam resumos sem payload de montagem; a leitura detalhada ocorre só ao abrir. Falha na lista não impede usar o filme, e Tentar novamente consulta somente jobs. Revisão de job legado cancela em mudança de conta/rota, sem restaurar estado antigo. Rascunhos e exportações anteriores preservados.
- Navegação reutiliza a verificação de perfil/equipe somente dentro da transição atual: de seis para duas consultas nas rotas comuns. Próxima navegação revalida permissões. Respostas atrasadas de configuração/empresas/projetos/orçamentos/comissões não repovoam uma conta diferente. Miniaturas também reconhecidas em `metadata.thumbnail_path`; artes sem miniatura ainda dependem do download do original.
- Falhas parciais ao carregar pastas/artes/projetos da empresa agora são erros contextuais antes de substituir os catálogos, nunca falsa lista vazia. Leituras de comissões compartilham prazo de 15 segundos com abort, sem repetir a RPC; erro é indisponível/null, não saldo zero. Teste VM de ciclo de leitura cobre 13 grupos, incluindo nova navegação revalidando perfil desativado.
- Nova comparação pedida pelo usuário: TIFF Photoshop `filme-completo-recortado (24).tif` versus sistema `hhhhhhhhhhhhh.tif`, ambos 6773×2811/300 DPI. Todos os 19.038.903 pixels do Spot idênticos; 11.286.046 pixels transparentes com CMYK zero/Spot255 e 2.215.005 parciais corretos. CMYK desvio máximo 2/255. Photoshop mantém camada alpha RLE (tag37724); sistema salva composto sem camadas editáveis, explicando fundo branco visual. Não modificar a máscara correta para imitar o quadriculado. RIP deve respeitar `Cor Spot 1`; impressão física ainda precisa de prova. Relatório privado em `tmp/spot-diagnostic/DIAGNOSTICO-TIFF-USUARIO-20260920.md`.
- Primeiro candidato passou Node e Edge desktop/celular: adição no mesmo turno, DOM preservado, zero SELECT ao adicionar com decode bloqueado; retry apenas jobs; PNG full/recorte idêntico, TIFF Spot exato, exclusões/rotações/rascunhos. Relatório `z19-film-results-vJQAma/report.json` no Temp. Retestar candidato final após demais guardas.
- Usuário autorizou explicitamente concluir, atualizar GitHub e publicar Vercel mesmo ausente. Login Vercel reautenticado com sucesso em 20/09/2026 como `ussloja-9450`; projeto e team conferidos. Aguardando publicação e verificação do candidato final — não confundir autenticação com deploy concluído.
- Gate final local concluído: 45 suítes Node + validação estática, Edge do filme desktop/celular com zero erros JS e requests externos na fixture. Relatório `z19-film-results-1hfVmp/report.json`; build `z19p-v21710-ghzc3o`. Hash SHA256 produção compilada `68f658876d82660cd5109f6f055f952f95871820f7a64db502591a245e6bd2f3`. Reuso de 40 imagens no seletor também passou. Smoke de login/páginas públicas carregou sem imagens quebradas ou overflow; não se utilizou a sessão real nem se fez impressão física.
- **Entrega confirmada:** implementação `f299278eb84ee0662ce687681751a201926e5f86` enviada para `main` no GitHub. Preview `dpl_5DSRiStj4iWUbW1YBrEas4FAvKpr` repetiu as 45 suítes na Vercel, com hashes idênticos ao build local; validado em navegador isolado usando acesso temporário oficial, sem desligar proteção. Promoção criou deployment de produção `dpl_9A9VGp52Z3pbMcwcEHgWKxdYS2cp`, READY, URL `019-personalizacoes-mxbh94s4j-uniquess.vercel.app` e aliases oficiais `019-personalizacoes.vercel.app` / `019-personalizacoes-uniquess.vercel.app`.
- Pós-deploy: domínio canônico HTTP200/meta2.17.10; HTML e 12 módulos essenciais idênticos byte a byte aos testados. Smoke executável no domínio oficial: login desktop1280/celular390, qualidades, portfólio e redirecionamentos comercial/demo, zero exceções JS, imagens quebradas e overflow. Runtime logs de erro/fatal do deployment não retornaram registros (app estático; isso não observa toda sessão autenticada de usuário). Não prometer ausência universal de bugs, velocidade instantânea da rede nem equivalência física da impressão.
...
- Entrega local: commit de implementação `9504208`, enviado à branch `codex/v2.17.9-film-fixes` do repositório existente. `main` remota não substituída. `http://localhost:8080/` confirmou versão 2.17.9 e bytes idênticos ao build em dez recursos, inclusive módulo de produção, preview, conversor, ICC e WASM.
- QA final do build real, sem patches experimentais: `scripts/film-browser-test.mjs`, exit 0, desktop 1280×900 e celular 390×844; zero erros JS, imagens quebradas, overflow ou requests externos na fixture. PNG integral/recorte idênticos em todos os bytes; TIFF integral 6850×1279 e recortado 3094×1279 com Spot exato. Relatório local em `z19-film-results-VjmAfV/report.json` no Temp do usuário; testes reproduzíveis versionados, fixtures não publicadas.
- Após o push da branch, a consulta Vercel ainda mostrou apenas deployments anteriores de 18/09. Nenhuma prévia nova nem produção 2.17.9 confirmada. Próximo passo online requer reautenticar Vercel (`vercel login`) e validar preview antes de promover; não contornar autenticação nem modificar a proteção do deployment.
