...
## Infraestrutura e produção de referência
...
O vercel.json passa a declarar build/outputDirectory e usa regras de header simples válidas, substituindo a expressão inválida historicamente contornada em deploys manuais. Scripts, SQL, metadados e memória não entram no output público.

### Validação necessária
Sintaxe, testes existentes de letras/rotação/jobs/rascunhos/filas, regressão nova v2.17.6, comparação de fonte sem/com acentos, SVG com sinal acima, espaçamento de DAVID, duas cópias de rascunho e falha de gravação. Preservar PNG único 300 DPI, transparência, dimensões e ausência de TIFF novo. Testes com dados simulados não substituem validação da conta real nem impressão física.
