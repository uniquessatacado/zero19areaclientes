...
export async function filmExportIdentity(items,layout,{digest=bytes=>crypto.subtle.digest('SHA-256',bytes)}={}){
  const artifact={version:1,width:layout.filmWidthMm,length:layout.lengthMm,
    items:items.map(item=>({id:item.localId,type:item.type,sourceId:item.sourceId,path:item.path||null,width:item.widthCm,height:item.heightCm,quantity:item.quantity,orderLink:item.orderLink||null,
...
export function linkedFilmQuantities(items,layout){
  ...
  return items.filter(item=>item.orderLink).map(item=>{
    ...
    if(![item.localId,link.quoteItemId,link.projectId,item.sourceId].every(id=>uuidPattern.test(String(id)))||quantity<1)throw new Error('Vínculo de produção incompleto. Adicione a arte novamente pela lista de pedidos.');
    return {film_item_id:item.localId,quote_item_id:link.quoteItemId,project_id:link.projectId,asset_id:item.sourceId,quantity,width_cm:item.widthCm,height_cm:item.heightCm};
...
export function pendingQuantity(row,items){
  ...
  const inDraft=items.filter(item=>item.orderLink?.quoteItemId===row.quote_item_id).reduce((sum,item)=>sum+Math.max(0,Number(item.quantity||0)-(exported.get(item.localId)||0)),0);
...
export function filmItemFromOrder(row,quantity=Number(row.remaining_quantity)){
  if(![row.project_id,row.quote_id,row.quote_item_id,row.asset_id,row.workspace_id].every(id=>uuidPattern.test(String(id)))||!(row.processed_path||row.original_path))throw new Error('Pedido sem identificação ou arquivo final válido. Atualize a lista.');
  ...
  return {localId:crypto.randomUUID(),type:'asset',sourceId:row.asset_id,label:`${row.company_name} • ${row.asset_name||row.label}`,quantity,widthCm:Number(row.width_cm),heightCm:Number(row.height_cm),path:row.processed_path||row.original_path,
    ...
    orderLink:{projectId:row.project_id,quoteId:row.quote_id,quoteItemId:row.quote_item_id}};
}
