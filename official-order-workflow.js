...
export function createOfficialOrderWorkflow(ctx){
  ...
  async function savePlacement(asset,workspace,project,product,choice,position,widthCm,heightCm){
  ...
  async function openPlacementWizard(asset,{workspace=state().currentWorkspace,project=null}={}){
    ...
    const modal=document.createElement('div');modal.className='official-placement-backdrop';let selectedProduct=products[0]||null,model=modelFrom(selectedProduct||{}),color=colorFrom(selectedProduct||{}),size=selectedProduct?.size||'',category=selectedProduct?.category||'',surface='front',position=null,ratio=(asset.width&&asset.height?asset.width/asset.height:1),widthCm=9,heightCm=9/ratio,busy=false;
    const chooseDefault=()=>{const available=positions.filter(p=>p.surface===surface&&fits(model,surface,widthCm,heightCm));position=available[0]||null};chooseDefault();
    const draw=()=>{
      const availableSurfaces=['front','back','left_sleeve','right_sleeve'].filter(s=>fits(model,s,widthCm,heightCm)),surfacePositions=positions.filter(p=>p.surface===surface&&fits(model,surface,widthCm,heightCm));if(!availableSurfaces.includes(surface)){surface=availableSurfaces[0]||'front';position=null}if(!position||position.surface!==surface||!surfacePositions.some(p=>p.id===position.id))position=surfacePositions[0]||null;
      ...
      modal.querySelector('[data-model]').onchange=e=>{model=e.target.value;chooseDefault();draw()};modal.querySelector('[data-color]').onchange=e=>{color=e.target.value;draw()};modal.querySelector('[data-size]').oninput=e=>{size=e.target.value};modal.querySelector('[data-category]').oninput=e=>{category=e.target.value};
